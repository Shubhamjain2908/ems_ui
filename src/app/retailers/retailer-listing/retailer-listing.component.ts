...
@Component({
  ...
  templateUrl: './retailer-listing.component.html',
  styleUrls: ['./retailer-listing.component.scss'],
  providers: [RetailersService],
  encapsulation: ViewEncapsulation.None
})

export class RetailerListingComponent implements OnInit {
  public data: any = [];
    ...
    this._httpService.delete(data.id)
      .subscribe((result: any) => {
        if (result.success === true) {
          ...
          this.data.splice(this.data.retailers.indexOf(data), 1);
          this.data.map(p => {
