...
export function parentIdValidator(control: FormGroup) {
  const type = control.get('level').value;
  const parentId = control.get('parentId').value;
  return (type === 1 || type === 2) && parentId === null ? { 'parentIdError': true } : null;
}

export function percentageValidator(flagKey: string = 'type', flagValue: any = 'percentage', valueKey: string = 'value'): ValidatorFn {
  return (control: FormGroup): any => {
    const flag = control.get(flagKey).value === flagValue;
    const value = control.get(valueKey).value;
    return flag && (value < 0 || value > 100) ? { 'percentageError': true } : null;
  }
}
